...
import EaseVisualizer from '../../../components/EaseVisualizer.astro';
...
Pick any ease name (and `in`/`out`/`inOut`) to see its curve and preview it as
motion — the value marker climbs the graph exactly as the animation would, with
a live progress/value readout. Whatever string you land on is what you pass to
`ease "…"`. This explorer is **GSAP-powered** (it samples the real ease function
in the browser), so it also covers the plugin eases — `rough`, `slow`,
`expoScale` (EasePack) and `CustomEase`/`CustomBounce`/`CustomWiggle` — which you
must `registerPlugin` to use at runtime.
