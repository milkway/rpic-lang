...
The dot marks exactly the point `H` remembered. (`dot(…)` isn't core pic —
the example *defines its own vocabulary* with a one-line macro, the pic way;
the circuit library ships one, and a native `dot` is under consideration.)
