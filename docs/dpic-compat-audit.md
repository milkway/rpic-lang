...
| `close` path command | Not a dpic command in 2025.08.01. `line right then up close` reports `close` as a missing variable. | Same practical result: no `close` command. | Do not implement for dpic parity. A future `close` would be an explicit rpic extension. |
...
rpic also treats `close` as an unresolved variable. Polygonal paths should be
closed in the classic pic style:
...
| Native `close` extension | Low | Not dpic parity. Only consider if it improves rpic ergonomics without changing classic input. |
