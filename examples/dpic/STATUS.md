...
- **54** examples render faithfully and are included in this gallery (including
  `man31` and the `copy`-assembled 3-D `EscherCube`).
...
- **7** still exercise dpic-specific extensions rpic does not yet implement,
  grouped below.

Corpus pass rate is now **65 / 72** (was 32 at the first audit).

## Not yet supported — 7

| Feature missing | Examples | Tracking |
|---|---|---|
| dpic m4 metaprogramming: `exec` run in the calling macro's argument scope (dynamic `$n` — needed to populate arrays via `array(…)`), m4 token pasting (`$1$2`), `sh`-command shell escapes — also pulled in by `copy` | `dpictools` (×2), `arrowwide`, `Spiral`, `tgraph`, `circles` (also PSTricks) | [#24](https://github.com/milkway/rpic-lang/issues/24) |
| Macro argument used in an expression slot (`{i}th` ncount) | `man35` | [#13](https://github.com/milkway/rpic-lang/issues/13) |

## Recently implemented

Pass count over this corpus rose **32 → 65**. Major features:
